...
function normalize(data) {
    ...
    for(let i = 0; i < keys.length; i++){
        if(typeof data[keys[i]].value == 'object' && data[keys[i]].value != null ){
            const datas = []
            data[keys[i]].value.map(e => {
                const dataValue = new Map()
                const nestedKeys = Object.keys(e)
                for(let k = 0; k < nestedKeys.length; k++){
                    dataValue.set(nestedKeys[k],e[nestedKeys[k]].value)
                   
                }
                datas.push(Object.fromEntries(dataValue))
            })
            newObj[keys[i]] = datas
        }else{
            newObj[keys[i]] = data[keys[i]].value
        }
    }
